<?php
session_start();
include("../../config/db.php"); // Pastiin path ini udah bener ya
...
// Buat nampilin pesan sukses/error (setelah operasi CRUD dari halaman lain)
...
// --- HANDLE AKSI HAPUS (lewat GET request dari link) ---
if (isset($_GET['action']) && $_GET['action'] === 'hapus') {
    ...
    header("Location: produk.php"); // Redirect balik ke halaman ini
    exit;
}

// --- Logika Kategori & Produk untuk Tampilan Tabel ---
// Kategori yang dipilih buat filter, defaultnya 'Semua'
$kategori_terpilih = $_GET['kategori'] ?? 'Semua';

// Ambil semua kategori unik dari DB, tambahin 'Semua' manual di awal
...
$sql_produk = "SELECT id_produk, nama_produk, harga, kategori FROM produk"; // Gak ada field deskripsi
...
$sql_produk .= " ORDER BY nama_produk ASC"; // Urutin berdasarkan nama produk
...
<head>
    ...
    <style>
        /* CSS-nya ini, jangan diubah kalo gak ngerti ya */
        :root {
            ...
            --color-error: #EF5350;
            --color-info: #2196F3;
            --color-delete: #DC3545; /* Merah untuk hapus */
            --color-edit: #FFC107; /* Kuning untuk edit */
            --color-add: #28A745; /* Hijau untuk tambah (ini nanti diganti) */
        }
        ...
        .add-product-btn-wrapper {
            /* text-align: center; Ganti dari center jadi left atau hilangkan */
            margin-bottom: 20px;
            /* Tambah padding kiri kalo mau mepet banget ke kiri */
            padding-left: 20px; /* Contoh: geser sedikit ke kanan dari pinggir layar */
        }
        .add-product-btn {
            background-color: var(--color-secondary-brown); /* Ganti warna dari hijau ke coklat */
            color: var(--color-text-light);
            ...
            cursor: pointer;
            transition: background-color 0.3s ease;
            text-decoration: none; /* Biar kayak tombol tapi link */
            display: inline-flex; /* Biar icon sama teks sebaris */
            align-items: center; /* Biar icon & teks di tengah vertikal */
            gap: 8px; /* Jarak antara icon dan teks */
        }
        .add-product-btn:hover {
            background-color: var(--color-primary-brown); /* Ganti warna hover */
        }
        ...
        .action-buttons {
            display: flex;
            gap: 8px;
            flex-wrap: wrap; /* Biar tombol gak keluar kalo layarnya kecil */
        }
        .action-buttons .btn {
            padding: 8px 12px;
            border: none;
        ...
        .action-buttons .btn-edit {
            background-color: var(--color-edit); /* Kuning */
        }
        .action-buttons .btn-edit:hover {
            background-color: #E0A800; /* Kuning lebih gelap */
        }
        .action-buttons .btn-delete {
            background-color: var(--color-delete); /* Merah */
        }
        ...
        @media screen and (max-width: 768px) {
            .category-nav { justify-content: flex-start; } /* Balikin ke kiri kalo kecil */
            ...
            .action-buttons { justify-content: flex-end; } /* Pindah tombol aksi ke kanan */
        }
    </style>
</head>
<body>
